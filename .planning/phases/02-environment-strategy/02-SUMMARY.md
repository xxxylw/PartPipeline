...
- Ran probes in `p3sam` and `holopart`.
...
- `p3sam`: PyTorch 2.4.0+cu124, has `torch_scatter`, missing `torch_cluster`.
- `holopart`: PyTorch 2.1.0+cu121, has `torch_cluster`, missing `torch_scatter`.
- HoloPart import smoke tests pass in `holopart` and fail in `p3sam`.
- SAMPart3D is not proven fully runnable yet because core import still reports `libnvrtc.so.12` loader-path issues in `p3sam`; this is a Phase 3 integration concern, not a shared-env blocker.
